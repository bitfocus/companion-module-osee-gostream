...
export function create(_deck: StreamDeck): CompanionPresetDefinitions {
	...
	presets[`Record_0`] = {
		type: 'button',
		category: 'Record',
		name: 'Start/Stop PGM Recording',
		style: {
			text: `Record\n$(gostream-iso:record_duration_hm)`,
			size: '14',
		...
		steps: [
			{
				down: [
					{
						actionId: Record.ActionId.PGMStartRecord,
						options: {
							record: 2,
						},
					},
		...
		feedbacks: [
			{
				feedbackId: FeedbackId.PGMRecord,
				options: {},
				style: {
					bgcolor: combineRgb(0, 0, 0),
					color: combineRgb(255, 255, 255),
					text: `Record\n$(gostreamdeck:record_cur_duration_hm)`,
					// png64: pngRecordingIcon_B64,
				},
			},
			{
				feedbackId: FeedbackId.RecordSSDAbnormalInfo,
				options: {},
				style: {
	...
	presets[`Record_1`] = {
		...
		steps: [
			{
				down: [
					{
						actionId: Record.ActionId.ISOStartRecord,
						options: {
							record: 2,
						},
					},
		...
		feedbacks: [
			{
				feedbackId: FeedbackId.ISORecord,
				options: {},
				style: {
					bgcolor: combineRgb(0, 0, 0),
					color: combineRgb(255, 255, 255),
			...
			{
				feedbackId: FeedbackId.ISORecordSSDAbnormalInfo,
				options: {},
				style: {
					bgcolor: combineRgb(110, 110, 110),
					color: combineRgb(255, 255, 64),
	...
	var qualitys= [
				...
				{ id: 2, label: 'good' },
				{ id: 3, label: 'hight' },]
	for (const qual of qualitys) {
		presets[`Record_Quality_${qual.id}`] = {
			type: 'button',
			category: 'Record',
			name: `Set recording quality to ${qual.label}`,
			style: {
				text: `Rec Quality ${qual.label}`,
				size: 'auto',
				color: combineRgb(255, 255, 255),
				bgcolor: combineRgb(0, 0, 0),
			},
			steps: [
				{
					down: [
						{
							actionId: Record.ActionId.RecordBitrate,
							options: {
								quality: qual.id,
							},
						},
